...
//-------------Atrybuty wierzcholkow------------------------------------------
  ...
  GLfloat ver_cone[74*3];
  GLfloat col_cone[74*3];
  float tmp_x,tmp_y,tmp_z, clr_x, clr_y, clr_z;
  float r = 0.4f;

	GLfloat ver_rectangle[] = {
    // wspolrzedne wierzcholkow prostokata
    // przednia sciana
    -1.0f, -0.5f, 0.0f,  // 0 lewy gorny
    1.0f, -0.5f, 0.0f,   // 1 prawy gorny
    -1.0f, -1.0f, 0.0f,  // 2 lewy dolny
    1.0f, -1.0f, 0.0f,   // 3 prawy dolny
                         // tylnia sciana
    -1.0f, -0.5f, -1.4f, // 4 lewy gorny
    1.0f, -0.5f, -1.4f,  // 5  prawy gorny
    -1.0f, -1.0f, -1.4f, // 6 lewy dolny
    1.0f, -1.0f, -1.4f,  // 7 prawy dolny
};
...
GLuint elements[] = {
    // prostokat skladamy z dwoch trojkatow
    0,
    1,
    2, // indeksy wierzcholkow dla pierwszego trojkata
    1,
    2,
    3, // indeksy wierzcholkow dla drugiego trojkata
...
int initShaders(void)
{
    ...
    for(int i=1;i<72;i++)
    {
      ...
      ver_cone[i*3] = tmp_x;
      ver_cone[i*3+1] = tmp_y;
      ver_cone[i*3+2] = tmp_z;
      col_cone[i*3] = clr_x;
      col_cone[i*3+1] = clr_y;
      col_cone[i*3+2] = clr_z;
      if(i==1)
      {
        ver_cone[72*3] = tmp_x;
        ver_cone[72*3+1] = tmp_y;
        ver_cone[72*3+2] = tmp_z;
        col_cone[72*3] = clr_x;
        col_cone[72*3+1] = clr_y;
        col_cone[72*3+2] = clr_z;
      }
      if(i==2)
      {
        ver_cone[73*3] = tmp_x;
        ver_cone[73*3+1] = tmp_y;
        ver_cone[73*3+2] = tmp_z;
        col_cone[73*3] = clr_x;
        col_cone[73*3+1] = clr_y;
        col_cone[73*3+2] = clr_z;
      }
    }
...
int initGL(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(ver_cone), ver_cone, GL_STATIC_DRAW); 	//skopiowanie danych do pamieci aktywnego bufora
...
int drawGLScene(int counter)
{
    ...
    glBindVertexArray(vao[1]);
    glDrawElements(GL_TRIANGLES, sizeof(ver_rectangle), GL_UNSIGNED_INT, 0); 
    

    float dy = 0;
    const int number_of_cone = 7;
    glm::mat4 scaleMatrix;
    for(int i=0;i<number_of_cone;i++)
    {
      scaleMatrix = glm::scale(glm::mat4(), glm::vec3(pow(2,-i/2), pow(2,-i/2), pow(2,-i/2))); 
      translationMatrix = glm::translate(glm::mat4(), glm::vec3(0.0f, dy, 0.0f));  	
      ...
      glDrawArrays(GL_TRIANGLE_FAN, 0, sizeof(ver_cone));
    }
